...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  */
    ...
    q_t = q_t.IntegrateBodyRate(V3D(gyro.x, gyro.y, gyro.z), dtIMU);

    float predictedRoll = q_t.Roll();
    float predictedPitch = q_t.Pitch();
    ekfState(6) = q_t.Yaw();
    
    // normalize yaw to -pi .. pi
    float twoPi = 2.f * F_PI;
    if (ekfState(6) > F_PI) ekfState(6) -= twoPi;
    if (ekfState(6) < -F_PI) ekfState(6) += twoPi;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F accels_inertial_frame = attitude.Rotate_BtoI(V3F(accel.x, accel.y, accel.z));
  predictedState(3) += accels_inertial_frame[0] * dt;
  predictedState(4) += accels_inertial_frame[1] * dt;
  predictedState(5) += (accels_inertial_frame[2] - 9.81f) * dt;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float twoPi = 2.f * F_PI;
  if (delta > F_PI) zFromX(0) += twoPi;
  else if (delta < -F_PI) zFromX(0) -= twoPi;
